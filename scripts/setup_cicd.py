...
def configure_git():
    ...
    try:
        ...
        subprocess.run(['git', 'config', 'user.name', 'BRS-KB CI/CD'],
                      check=True, capture_output=True)
        subprocess.run(['git', 'config', 'user.email', 'cicd@brs-kb.local'],
                      check=True, capture_output=True)
...
def main():
    ...
    if success_count == len(setup_steps):
        ...
        print(f"⚠️ {len(setup_steps) - success_count} components failed setup")
